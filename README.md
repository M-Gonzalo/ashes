...
- The search and filtering are performed via Ash read actions against the local SQLite database. If you change the DB or resource shape, update the read action accordingly.
- The screenshots are static assets intended purely for preview; they are not generated by the app at runtime.

If you'd like more detail (example commands, docker-compose for DB, or annotated screenshots), tell me what to add.